...
const TONES = {
  ...
  blue:    { bg: 'bg-blue-50',    text: 'text-blue-600',    ring: 'ring-blue-200/60',    dot: 'bg-blue-500' },
};
  ...
  if (menu.big) {
    return (
      <div onClick={onClick} className={`${BASE} p-4 sm:p-5 sm:col-span-2 lg:col-span-2`}>
        <div className="absolute -top-12 -right-10 h-40 w-40 rounded-full bg-blue-50/70 blur-2xl" />
        <div className="relative flex items-start gap-3 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <ArrowUpRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-blue-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </div>
    ...
    return (
      <div onClick={onClick} className={`${BASE} p-4 sm:col-span-2 lg:col-span-3`}>
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          <button className="sm:ml-auto h-9 px-3 rounded-lg border border-slate-200 hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700 text-[12px] font-semibold text-slate-700 flex items-center justify-center gap-1.5 transition-colors shrink-0">
...
const ActivityTimeline = ({ logs, loading, onRefresh }) => {
  ...
  const filters = ['Semua', 'ADD', 'DD', 'BHPRD'];
...
const KKDPage = () => {
  ...
  const fetchActivityLogs = useCallback(async () => {
    try {
      ...
      const res = await api.get('/bidang/4/activity-logs');
  ...
  const fetchDashboard = async () => {
    try {
      setLoading(true);
      const res = await api.get('/bidang/4/dashboard');
  ...
  const menus = [
    {
      id: 'add', big: true, tone: 'emerald',
      title: 'Alokasi Dana Desa', short: 'ADD',
      desc: 'Kelola data penyaluran dan monitoring ADD per desa di seluruh kecamatan.',
      icon: DollarSign,
      route: getPath('/kkd/add'),
      stat: { value: fmtOr(stats.penerima_add, '416'), label: 'desa penerima' },
    },
    {
      id: 'dd', tone: 'violet',
      title: 'Dana Desa', short: 'DD',
      desc: 'Penyaluran dan realisasi Dana Desa dari APBN',
      icon: TrendingUp,
      route: getPath('/kkd/dd'),
      stat: { value: fmtOr(stats.penerima_dd, '416'), label: 'desa' },
    },
    {
      id: 'bhprd', tone: 'blue',
      title: 'BHPRD', short: 'BHPRD',
  ...
  const statsStrip = [
    ...
    { label: 'Penerima ADD', value: fmtOr(stats.penerima_add, '416'), delta: 'desa aktif',         up: true },
    { label: 'Penerima DD',  value: fmtOr(stats.penerima_dd,  '416'), delta: 'dari APBN',          up: true },
    { label: 'Pagu 2026',    value: fmtRp(stats.pagu),                 delta: 'ADD · DD · BHPRD',   up: true },
  ];
      ...
      <div className="px-4 sm:px-6 pt-6 pb-5">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 lg:gap-6">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 flex items-center justify-center text-blue-600 shadow-[0_1px_0_rgba(0,0,0,0.02),_0_8px_24px_-12px_rgba(59,130,246,0.35)] shrink-0">
              <Landmark className="h-5 w-5 sm:h-6 sm:w-6" strokeWidth={1.75} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <span className="text-[10px] sm:text-[10.5px] font-bold tracking-[0.14em] uppercase text-blue-600">Bidang Kerja</span>
                <span className="h-1 w-1 rounded-full bg-slate-300" />
                <span className="text-[10px] sm:text-[10.5px] font-semibold tracking-wide text-slate-500">Bidang 04 · KKD</span>
              </div>
              <h1 className="text-[22px] sm:text-[26px] lg:text-[28px] leading-[1.1] font-extrabold tracking-tight text-slate-900">
                Bidang Keuangan &amp; Kekayaan Desa
              </h1>
              <p className="mt-1.5 text-[12.5px] sm:text-[13.5px] text-slate-500 max-w-xl">
                Kelola penyaluran ADD, Dana Desa, dan BHPRD untuk seluruh desa di Kabupaten Bogor.
              </p>
            </div>
          </div>
      ...
      <div className="px-4 sm:px-6 pb-10">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          <aside className="w-full lg:w-[340px] shrink-0 space-y-4">
            <DaftarPegawaiBidang bidangId={4} bidangName="Bidang KKD" />
            <ActivityTimeline
              logs={activityLogs}
