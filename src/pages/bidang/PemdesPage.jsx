...
import {
  Briefcase, Activity, Clock, Users,
  FileText, MapPinned, Scale, ChevronRight, ArrowUpRight, RotateCcw,
} from 'lucide-react';
import api from '../../api';
...
const TONES = {
  ...
  teal:    { bg: 'bg-teal-50',    text: 'text-teal-600',    ring: 'ring-teal-200/60',    dot: 'bg-teal-500' },
};
  ...
  if (menu.big) {
    return (
      <div onClick={onClick} className={`${BASE} p-4 sm:p-5 sm:col-span-2 lg:col-span-2`}>
        <div className="absolute -top-12 -right-10 h-40 w-40 rounded-full bg-teal-50/70 blur-2xl" />
        <div className="relative flex items-start gap-3 sm:gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              <ArrowUpRight className="h-3.5 w-3.5 text-slate-300 group-hover:text-teal-600 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform" />
            </div>
    ...
    return (
      <div onClick={onClick} className={`${BASE} p-4 sm:col-span-2 lg:col-span-3`}>
        <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4">
          ...
          <button className="sm:ml-auto h-9 px-3 rounded-lg border border-slate-200 hover:border-teal-300 hover:bg-teal-50 hover:text-teal-700 text-[12px] font-semibold text-slate-700 flex items-center justify-center gap-1.5 transition-colors shrink-0">
...
const ActivityTimeline = ({ logs, loading, onRefresh }) => {
  ...
  const filters = ['Semua', 'Aparatur', 'Hukum', 'Profil'];

  const filtered = useMemo(() => filter === 'Semua' ? logs : logs.filter(l => l.module?.toLowerCase().includes(filter.toLowerCase())), [logs, filter]);
                  ...
                  return (
                    <div key={log.id} className="relative pb-3 last:pb-0">
                      ...
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {log.module && <span className="text-[9.5px] font-mono text-slate-400 bg-slate-50 px-1 rounded">[{log.module}]</span>}
                      </div>
...
const PemdesPage = () => {
  ...
  const fmt    = n => n != null ? Number(n).toLocaleString('id-ID') : null;
  const fmtOr  = (n, fallback = '—') => fmt(n) ?? fallback;
  const fmtRp  = n => n != null && n > 0
    ? new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', notation: 'compact', maximumFractionDigits: 2 }).format(n)
    : 'Rp —';

  const menus = [
    {
      id: 'aparatur-desa', big: true, tone: 'teal',
      title: 'Aparatur Desa', short: 'Aparatur',
      desc: 'Kelola data perangkat dan aparatur desa seluruh kecamatan se-Kabupaten Bogor.',
      icon: Users,
      route: getPath('/pemdes/aparatur-desa'),
      stat: { value: fmtOr(stats.total_aparatur), label: 'aparatur terdaftar' },
    },
    {
      id: 'profil-desa', tone: 'emerald',
      title: 'Profil Desa', short: 'Profil',
      desc: 'Dashboard data dan profil seluruh desa',
      icon: MapPinned,
      route: getPath('/pemdes/profil-desa'),
      stat: { value: fmtOr(stats.total_desa, '416'), label: 'desa' },
    },
    {
      ...
      stat: { value: fmtOr(stats.total_produk_hukum), label: 'dokumen' },
    },
    {
      id: 'musdesus', wide: true, tone: 'violet',
      title: 'Musyawarah Desa Khusus', short: 'Musdesus',
      desc: 'Data, laporan, dan monitoring Musdes Khusus per desa se-Kabupaten Bogor',
      icon: FileText,
      route: '/core-dashboard/musdesus',
      stat: { value: 'Musdes', label: 'per desa · per periode' },
    },
  ];

  const statsStrip = [
    { label: 'Total Desa',    value: fmtOr(stats.total_desa, '416'), delta: 'se-Kabupaten Bogor',     up: true },
    { label: 'Perangkat Desa', value: fmtOr(stats.total_aparatur),   delta: 'kepala desa & staf',     up: true },
    { label: 'Produk Hukum',  value: fmtOr(stats.total_produk_hukum), delta: 'Perdes · Perkades · SK', up: true },
    { label: 'Pagu 2026',     value: fmtRp(stats.pagu),               delta: 'Program Pemdes',         up: true },
  ];
      ...
      <div className="px-4 sm:px-6 pt-6 pb-5">
        <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 lg:gap-6">
          <div className="flex items-start gap-3 sm:gap-4">
            <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-2xl bg-gradient-to-br from-teal-50 to-emerald-50 border border-teal-100 flex items-center justify-center text-teal-600 shadow-[0_1px_0_rgba(0,0,0,0.02),_0_8px_24px_-12px_rgba(20,184,166,0.35)] shrink-0">
              <Briefcase className="h-5 w-5 sm:h-6 sm:w-6" strokeWidth={1.75} />
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                <span className="text-[10px] sm:text-[10.5px] font-bold tracking-[0.14em] uppercase text-teal-600">Bidang Kerja</span>
                <span className="h-1 w-1 rounded-full bg-slate-300" />
                <span className="text-[10px] sm:text-[10.5px] font-semibold tracking-wide text-slate-500">Bidang 06 · Pemdes</span>
              </div>
              ...
              <p className="mt-1.5 text-[12.5px] sm:text-[13.5px] text-slate-500 max-w-xl">
                Kelola aparatur desa, profil desa, produk hukum, dan Musyawarah Desa Khusus seluruh wilayah.
              </p>
            </div>
          </div>
              ...
              <div className="sm:hidden bg-white border border-slate-200/80 rounded-2xl p-4 pb-5">
                <div className="grid grid-cols-4 gap-y-4 gap-x-1">
                  {menus.map(menu => {
                    ...
                    const Icon = menu.icon;
                    return (
                      <button key={menu.id} onClick={() => navigate(menu.route)}
