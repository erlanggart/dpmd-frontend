...
import { Briefcase, FileText, Users as UsersIcon, Activity, ArrowLeft, Clock, CheckCircle, AlertCircle } from 'lucide-react';
import api from '../../api';
...
const PemdesPage = () => {
	...
	const statsCards = [
		{
			title: 'Total Musdesus',
			value: stats.total_musdesus || 0,
			icon: FileText,
			color: 'from-teal-500 to-teal-600'
		},
		{
			title: 'Musdesus Approved',
			value: stats.musdesus_approved || 0,
			icon: CheckCircle,
			color: 'from-green-500 to-green-600'
		},
		{
			title: 'Musdesus Pending',
			value: stats.musdesus_pending || 0,
			icon: AlertCircle,
			color: 'from-yellow-500 to-yellow-600'
		},
		{
			title: 'Total Aparatur',
			value: stats.total_aparatur || 0,
			icon: UsersIcon,
			color: 'from-blue-500 to-blue-600'
		},
		{
			title: 'Total Produk Hukum',
			value: stats.total_produk_hukum || 0,
			icon: FileText,
			color: 'from-purple-500 to-purple-600'
		},
		{
			title: 'Produk Hukum Berlaku',
			value: stats.produk_hukum_berlaku || 0,
			icon: CheckCircle,
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
					<button
						...
						className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-lg hover:border-teal-300 transition-all text-left group"
					>
						<div className="flex items-center gap-4">
							<div className="h-14 w-14 bg-gradient-to-br from-teal-500 to-teal-600 rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
					...
					<button
						onClick={() => navigate('/core-dashboard/aparatur-desa')}
						className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 hover:shadow-lg hover:border-teal-300 transition-all text-left group"
					>
						<div className="flex items-center gap-4">
