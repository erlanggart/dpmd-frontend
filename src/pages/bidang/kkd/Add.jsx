// src/pages/bidang/kkd/Add.jsx - Alokasi Dana Desa (ADD) 2025
...
const Add = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      console.error('Error loading ADD data:', error);
  ...
  const exportToExcel = () => {
    try {
      const summaryData = [
        ['LAPORAN ALOKASI DANA DESA (ADD) 2025'],
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Laporan ADD 2025');
      
      const fileName = `Laporan_ADD_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post('/add/upload', formData, {
        headers: {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
          ...
          ADD (Alokasi Dana Desa) 2025
        </h1>
        <p className="text-gray-600 mt-2">
          Data alokasi dana desa untuk pembangunan dan pemberdayaan masyarakat
        </p>
      </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Update Data ADD</h3>
              <button
                onClick={() => {
                  setShowUploadModal(false);
                  setUploadFile(null);
