...
const DdNonEarmarkedT1 = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/dd-nonNon-Earmarked-t1/data');
      ...
      toast.error('Gagal memuat data DD Non-Earmarked Tahap 1');
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DD Non-Earmarked T1');
      XLSX.writeFile(wb, `DD_Non-Earmarked_T1_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post('/dd-nonNon-Earmarked-t1/upload', formData);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Dana Desa Non-Earmarked Tahap 1 Tahun 2025</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-6 flex flex-wrap gap-3 justify-between">
        <div className="flex gap-3">
          <button onClick={() => setShowUploadModal(true)} className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg border ${
              groupByKecamatan ? 'bg-indigo-50 border-indigo-300 text-indigo-700' : 'bg-white border-gray-300'
            }`}
          >
                  ...
                  <React.Fragment key={kecamatan}>
                    ...
                    {isExpanded && items.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right font-semibold text-indigo-600">{formatCurrency(item.realisasi)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleUpload} disabled={!uploadFile || uploading} className="flex-1 px-4 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 flex items-center justify-center gap-2">
...
export default DdNonEarmarkedT1;
