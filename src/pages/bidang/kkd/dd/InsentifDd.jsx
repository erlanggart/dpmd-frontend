...
const InsentifDd = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error('Gagal memuat data Insentif Dana Desa');
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'DD Earmarked T1');
      XLSX.writeFile(wb, `DD_Earmarked_T1_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post('/insentif-dd/upload', formData);
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Insentif Dana Desa Tahun 2025</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-6 flex flex-wrap gap-3 justify-between">
        <div className="flex gap-3">
          <button onClick={() => setShowUploadModal(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg border ${
              groupByKecamatan ? 'bg-emerald-50 border-emerald-300 text-emerald-700' : 'bg-white border-gray-300'
            }`}
          >
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden">
        <table className="w-full">
          <thead className="bg-gradient-to-r from-emerald-600 to-blue-700 text-white">
            <tr>
                  ...
                  <React.Fragment key={kecamatan}>
                    ...
                    {isExpanded && items.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right font-semibold text-emerald-600">{formatCurrency(item.realisasi)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleUpload} disabled={!uploadFile || uploading} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 disabled:opacity-50 flex items-center justify-center gap-2">
