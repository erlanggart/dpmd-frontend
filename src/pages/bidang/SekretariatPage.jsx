						...
						<div className="relative z-10">
							...
							<div>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
									<button
										onClick={() => navigate(['kepala_dinas', 'sekretaris_dinas'].includes(user.role) ? '/dpmd/disposisi' : getPath('/sekretariat/disposisi'))}
								...
								{(user?.role === 'superadmin' || Number(user?.bidang_id) === 2) && (
										<button
											onClick={() => navigate(getPath('/sekretariat/berita'))}
											className="group relative bg-white rounded-2xl shadow-lg hover:shadow-2xl border border-gray-100 p-6 hover:border-sky-300 transition-all duration-300 text-left overflow-hidden hover:-translate-y-1"
										>
											<div className="absolute inset-0 bg-gradient-to-br from-sky-400/5 to-blue-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
											<div className="relative flex items-center gap-5">
												<div className="h-16 w-16 bg-gradient-to-br from-sky-500 to-blue-600 rounded-2xl flex items-center justify-center group-hover:scale-110 group-hover:rotate-3 transition-all duration-300 shadow-lg shadow-sky-500/25">
													<Newspaper className="h-8 w-8 text-white" />
												</div>
												<div className="flex-1">
													<h3 className="font-bold text-gray-800 text-lg mb-1">Manajemen Berita</h3>
													<p className="text-sm text-gray-500">Publikasi artikel & lampiran PDF</p>
												</div>
												<ChevronRight className="h-6 w-6 text-gray-400 group-hover:text-sky-600 group-hover:translate-x-1 transition-all" />
											</div>
										</button>
									)}
								...
								{(user?.role === 'superadmin' || Number(user?.bidang_id) === 2) && (
									...
									{/* Anggaran Bidang - semua user bisa lihat */}
									<button
										onClick={() => navigate(getPath('/sekretariat/anggaran'))}
									...
									>
										<div className="absolute inset-0 bg-gradient-to-br from-emerald-400/5 to-emerald-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-300"></div>
										<div className="relative flex items-center gap-5">
											...
											<div className="flex-1">
												<h3 className="font-bold text-gray-800 text-lg mb-1">Anggaran Bidang</h3>
