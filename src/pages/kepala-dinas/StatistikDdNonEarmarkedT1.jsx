...
const StatistikDdNonEarmarkedT1 = () => {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/dd-nonearmarked-t1/data');
      setData(response.data.data || []);
      setError(null);
    } catch (err) {
      console.error('Error fetching DD Non-Earmarked Tahap 1 data:', err);
      setError('Gagal memuat data DD Non-Earmarked Tahap 1');
    } finally {
      setLoading(false);
    }
  };
  ...
  const statusChartData = {
    ...
    datasets: [{
      ...
      backgroundColor: [
        ...
        'rgba(236, 72, 153, 0.8)',  // violet
        'rgba(168, 85, 247, 0.8)',  // violet
        'rgba(219, 39, 119, 0.8)',  // fuchsia
        'rgba(192, 132, 252, 0.8)', // indigo light
      ],
      borderColor: [
        'rgba(147, 51, 234, 1)',
        'rgba(236, 72, 153, 1)',
  ...
  const handleExport = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Statistik DD Non-Earmarked Tahap 1');
    XLSX.writeFile(wb, `Statistik_DD Non-Earmarked Tahap 1_${new Date().toISOString().split('T')[0]}.xlsx`);
  };

  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-violet-50 flex items-center justify-center">
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-16 w-16 border-4 border-indigo-500 border-t-transparent"></div>
          <p className="mt-4 text-lg text-gray-600 font-medium">Memuat data DD Non-Earmarked Tahap 1...</p>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-violet-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-xl p-8 max-w-md w-full border-l-4 border-red-500">
          ...
          <button
            onClick={fetchData}
            className="w-full bg-gradient-to-r from-indigo-600 to-violet-600 text-white py-3 rounded-xl hover:from-indigo-700 hover:to-violet-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl"
          >
            Coba Lagi
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-indigo-50 to-violet-50 p-4 md:p-6 lg:p-8">
      <div className="max-w-7xl mx-auto">
        {/* Back Button */}
        <button
          onClick={() => navigate('/core-dashboard/dashboard')}
          className="group flex items-center gap-2 text-gray-600 hover:text-indigo-600 mb-6 transition-all duration-300 bg-white px-4 py-2 rounded-xl shadow-md hover:shadow-lg"
        >
        ...
        {/* Hero Header */}
        <div className="relative bg-gradient-to-r from-indigo-600 via-indigo-700 to-violet-700 rounded-3xl shadow-2xl p-8 mb-8 overflow-hidden">
          ...
          <div className="absolute top-1/2 right-1/4 w-40 h-40 bg-violet-400 opacity-20 rounded-full blur-3xl"></div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-3xl md:text-4xl font-bold text-white drop-shadow-lg">
                  Statistik DD Non-Earmarked Tahap 1
                </h1>
                <p className="text-indigo-100 mt-1">Dana Desa</p>
              </div>
            </div>
        ...
        <div className="relative bg-white rounded-2xl shadow-xl p-6 md:p-8 mb-8 overflow-hidden border border-gray-100">
          ...
          <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-indigo-100 to-violet-100 opacity-30 rounded-full -mr-32 -mt-32"></div>
          <div className="absolute bottom-0 left-0 w-48 h-48 bg-gradient-to-tr from-indigo-50 to-violet-50 opacity-50 rounded-full -ml-24 -mb-24"></div>
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              <div className="bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl p-6 border border-indigo-100 shadow-md hover:shadow-lg transition-shadow duration-300">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    ...
                    <p className="text-2xl md:text-3xl font-bold text-indigo-600">{totalDesa}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-gradient-to-br from-indigo-50 to-violet-50 rounded-xl p-6 border border-indigo-100 shadow-md hover:shadow-lg transition-shadow duration-300">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-base md:text-lg font-bold text-indigo-600 break-words">{formatRupiah(avgPerDesa)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 flex justify-end">
              <button
                ...
                className="bg-gradient-to-r from-indigo-600 to-violet-600 text-white px-6 py-3 rounded-xl hover:from-indigo-700 hover:to-violet-700 transition-all duration-300 font-semibold shadow-lg hover:shadow-xl flex items-center gap-2"
              >
                        ...
                        <td className="px-6 py-4" colSpan="4">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-sm font-semibold text-indigo-700">{formatRupiah(totalKecamatan)}</span>
                            </div>
                            <button
