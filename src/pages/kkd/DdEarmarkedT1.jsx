...
const DdEarmarkedT1 = () => {
  ...
  const fetchDdEarmarkedT1Data = async () => {
    try {
      setLoading(true);
      const response = await api.get('/dd-earmarked-t1/data');
      const data = response.data.data;
      
      // DdEarmarkedT1 workflow: Simple 1 record per desa
      // Data structure: {sts, kecamatan, desa, Realisasi}
      ...
      console.error('Error loading DdEarmarkedT1 data:', error);
      toast.error('Gagal memuat data DdEarmarkedT1');
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Laporan DdEarmarkedT1 2025');
      
      const fileName = `Laporan_DdEarmarkedT1_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const response = await api.post('/dd-earmarked-t1/upload', formData, {
        headers: {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
          <FiDollarSign as FiMoney className="text-cyan-600" />
          DdEarmarkedT1 (Dana Desa Earmarked Tahap 1) 2025
        </h1>
        <p className="text-gray-600 mt-2">
          Data Dana Desa Earmarked Tahap 1 untuk pembangunan dan pemberdayaan masyarakat
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-cyan-500">
          <div className="flex items-center justify-between">
            ...
            <FiMapPin className="text-4xl text-cyan-500" />
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-blue-500">
          <div className="flex items-center justify-between">
            ...
            <FiDollarSign as FiMoney className="text-4xl text-blue-500" />
          </div>
        </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-lg font-bold text-gray-900">
                {formatCurrency(stats.avgRealisasiPerDesa)}
              </p>
            </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <option value="realisasi">Urutkan: Alokasi Terbesar</option>
          </select>
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-cyan-600 to-cyan-700 text-white">
              <tr>
                ...
                <th className="px-6 py-4 text-right">Alokasi Dana</th>
              </tr>
            </thead>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-bold">Update Data DdEarmarkedT1</h3>
              <button
                onClick={() => {
                  setShowUploadModal(false);
                  setUploadFile(null);
