...
import { 
  Mail, Send, Clock, Check, Eye, Plus, Upload, X, FileText, 
  Calendar, Search, Filter, ChevronRight, Download, AlertCircle,
  CheckCircle2, XCircle, Inbox, ArrowRight, User, Building2
} from 'lucide-react';
import api from '../../api';
...
export default function DisposisiSuratModern() {
    ...
    return data.filter(item => {
      const matchSearch = searchQuery === '' || 
        (item.perihal?.toLowerCase().includes(searchQuery.toLowerCase()) ||
         item.surat?.perihal?.toLowerCase().includes(searchQuery.toLowerCase()) ||
         item.nomor_surat?.toLowerCase().includes(searchQuery.toLowerCase()) ||
         item.surat?.nomor_surat?.toLowerCase().includes(searchQuery.toLowerCase()));
  ...
  const paginatedDisposisiMasuk = getPaginatedData(filteredDisposisiMasuk, currentPageDisposisiMasuk);
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 lg:pb-8">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl sm:text-3xl font-bold text-white">Disposisi Surat</h1>
                <p className={`text-sm ${themeColors.textLight} mt-1`}>Kelola surat masuk dan disposisi Anda</p>
              </div>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        {/* Statistik Cards - Ultra Compact Design */}
        {statistik && (
          <div className="grid grid-cols-4 gap-2 mb-4">
            ...
            <div className="group bg-white rounded-lg p-2.5 shadow-sm hover:shadow-md transition-all duration-200 border border-yellow-100 hover:border-yellow-300">
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-bold text-gray-800">{statistik.masuk.pending}</p>
            ...
            <div className="group bg-white rounded-lg p-2.5 shadow-sm hover:shadow-md transition-all duration-200 border border-blue-100 hover:border-blue-300">
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-bold text-gray-800">{statistik.masuk.dibaca}</p>
            ...
            <div className="group bg-white rounded-lg p-2.5 shadow-sm hover:shadow-md transition-all duration-200 border border-purple-100 hover:border-purple-300">
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-bold text-gray-800">{statistik.masuk.proses}</p>
            ...
            <div className="group bg-white rounded-lg p-2.5 shadow-sm hover:shadow-md transition-all duration-200 border border-green-100 hover:border-green-300">
              <div className="flex flex-col items-center">
                ...
                <p className="text-xl font-bold text-gray-800">{statistik.masuk.selesai}</p>
          ...
          <div className="flex gap-1.5">
            {isSecretariat && (
              ...
              >
                ...
                <div className={`absolute inset-0 bg-gradient-to-br from-blue-500/10 to-cyan-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${activeTab === 'surat-masuk' ? 'opacity-100' : ''}`}></div>
            ...
            >
              {/* Background Gradient on Hover */}
              <div className={`absolute inset-0 bg-gradient-to-br from-green-500/10 to-emerald-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${activeTab === 'masuk' ? 'opacity-100' : ''}`}></div>
            ...
            >
              {/* Background Gradient on Hover */}
              <div className={`absolute inset-0 bg-gradient-to-br from-purple-500/10 to-pink-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${activeTab === 'keluar' ? 'opacity-100' : ''}`}></div>
            ...
            <div className="flex items-center gap-2 mt-3 pt-3 border-t">
              ...
              {filterStatus !== 'all' && (
                <span className="px-3 py-1 bg-purple-100 text-purple-700 rounded-lg text-sm font-medium flex items-center gap-1">
                  Status: {filterStatus}
                  <button onClick={() => setFilterStatus('all')} className="hover:bg-purple-200 rounded-full p-0.5">
                    <X className="h-3 w-3" />
                  </button>
                </span>
              )}
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-600 mt-4 font-medium">Memuat data disposisi...</p>
          </div>
        ) : (
          <div className="space-y-4">
              ...
              <>
                {filteredDisposisiMasuk.length === 0 ? (
                  <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                    ...
                    <h3 className="text-xl font-bold text-gray-800 mb-2">Tidak Ada Disposisi Masuk</h3>
                          ...
                          <div className="p-3">
                            ...
                            <div className="flex items-start gap-2 mb-2">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-gray-500 mt-0.5">{disposisi.surat?.nomor_surat || '-'}</p>
                              </div>
                            ...
                            <div className="flex items-center gap-3 text-xs text-gray-600 mb-2">
                              <div className="flex items-center gap-1">
                                ...
                                <span className="truncate max-w-[120px]">{disposisi.dari_user?.name || 'Unknown'}</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <Calendar className="h-3 w-3" />
                                <span>{new Date(disposisi.tanggal_disposisi).toLocaleDateString('id-ID', { day: 'numeric', month: 'short' })}</span>
                              </div>
                            </div>
                ...
                {filteredDisposisiMasuk.length > itemsPerPage && (
                  <div className="flex items-center justify-between bg-white rounded-2xl shadow-lg p-4 mt-6">
                    <p className="text-sm text-gray-600">
                      Menampilkan {((currentPageDisposisiMasuk - 1) * itemsPerPage) + 1} - {Math.min(currentPageDisposisiMasuk * itemsPerPage, filteredDisposisiMasuk.length)} dari {filteredDisposisiMasuk.length} data
                    </p>
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() => setCurrentPageDisposisiMasuk(prev => Math.max(1, prev - 1))}
                      ...
                      <button
                        onClick={() => setCurrentPageDisposisiMasuk(prev => prev + 1)}
                        disabled={currentPageDisposisiMasuk >= getTotalPages(filteredDisposisiMasuk)}
                        className="px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-all"
                      >
                        Next
                      </button>
            ...
            {activeTab === 'keluar' && (
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                <Send className="mx-auto h-16 w-16 text-purple-400 mb-4" />
                <h3 className="text-xl font-bold text-gray-800 mb-2">Disposisi Keluar</h3>
                <p className="text-gray-500">Menampilkan disposisi yang Anda kirim</p>
              </div>
            )}
            ...
            {activeTab === 'surat-masuk' && isSecretariat && (
              <div className="bg-white rounded-2xl shadow-lg p-12 text-center">
                ...
                <h3 className="text-xl font-bold text-gray-800 mb-2">Surat Masuk</h3>
                <p className="text-gray-500">Menampilkan surat masuk yang belum didisposisi</p>
              </div>
            )}
          </div>
        )}
