...
const BankeuT1 = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      toast.error('Gagal memuat data Bantuan Keuangan Tahap 1');
      setLoading(false);
    }
  };
  ...
  const exportToExcel = () => {
    try {
      const exportData = filteredData.map((item, index) => ({
        ...
        'Desa': item.desa,
        'Status': item.status,
        'Alokasi': item.realisasi
      }));

      const ws = XLSX.utils.json_to_sheet(exportData);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, 'Bankeu T1');
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mt-2">Bantuan Keuangan Kepada Desa Tahap 1 Tahun 2025</p>
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md mb-6 flex flex-wrap gap-3 justify-between">
        <div className="flex gap-3">
          <button onClick={() => setShowUploadModal(true)} className="flex items-center gap-2 px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <button
            ...
            className={`flex items-center justify-center gap-2 px-4 py-2 rounded-lg border ${
              groupByKecamatan ? 'bg-cyan-50 border-cyan-300 text-cyan-700' : 'bg-white border-gray-300'
            }`}
          >
        ...
        <table className="w-full">
          <thead className="bg-gradient-to-r from-cyan-600 to-blue-700 text-white">
            <tr>
              ...
              <th className="px-6 py-4 text-right">Alokasi (Rp)</th>
            </tr>
          </thead>
                  ...
                  <React.Fragment key={kecamatan}>
                    ...
                    {isExpanded && items.map((item, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-right font-semibold text-cyan-600">{formatCurrency(item.realisasi)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                );
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleUpload} disabled={!uploadFile || uploading} className="flex-1 px-4 py-2 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 disabled:opacity-50 flex items-center justify-center gap-2">
