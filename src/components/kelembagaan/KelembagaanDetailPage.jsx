...
export default function KelembagaanDetailPage({ isAdminView: propIsAdminView }) {
	...
	const loadDetail = useCallback(async () => {
		...
		try {
			...
			if (type === "rw") {
				...
				if (data?.rts) {
					setAnak(data.rts);
					console.log('📊 [loadDetail] RT list from RW response:', data.rts.length, 'items');
				}
			} else if (type === "rt") {
	...
	const handleToggleStatus = async (kelembagaanId, currentStatus) => {
		...
		const result = await showConfirmAlert(
			...
			`Apakah Anda yakin ingin mengubah status menjadi ${newStatus === "aktif" ? "Aktif" : "Tidak Aktif"}?`,
			"warning"
		);
	...
	const handleToggleVerification = async (kelembagaanId, currentStatus) => {
		...
		const result = await showConfirmAlert(
			...
			`Apakah Anda yakin ingin ${newStatus === "verified" ? "memverifikasi" : "membatalkan verifikasi"} kelembagaan ini?`,
			"warning"
		);
		...
		<div className="min-h-full">
			...
			<div className="bg-white rounded-lg shadow-sm border border-gray-200 mb-4 px-4 py-3">					<div className="flex items-center justify-between mb-2">
					...
					{type === 'rt' && detail?.rws ? (
						<>
							<Link
								to={isAdmin ? `/dashboard/kelembagaan/admin/${routeDesaId || targetDesaId}/rw` : `/desa/kelembagaan/rw`}
								className="text-gray-500 hover:text-indigo-600 transition-colors"
							>
								RW
							</Link>
							<FaChevronRight className="text-gray-400 text-xs" />
							<Link
								to={isAdmin ? `/dashboard/kelembagaan/admin/${routeDesaId || targetDesaId}/rw/${detail.rw_id}` : `/desa/kelembagaan/rw/${detail.rw_id}`}
								className="text-gray-500 hover:text-indigo-600 transition-colors"
							>
								{detail.rws?.nomor || 'RW'}
							</Link>
							<FaChevronRight className="text-gray-400 text-xs" />
							<Link
								to={isAdmin ? `/dashboard/kelembagaan/admin/${routeDesaId || targetDesaId}/rt` : `/desa/kelembagaan/rt`}
								className="text-gray-500 hover:text-indigo-600 transition-colors"
							>
								RT
							</Link>
						...
						<>
							<Link
								to={isAdmin ? `/dashboard/kelembagaan/${type}` : `/desa/kelembagaan/${type}`}
								className="text-gray-500 hover:text-indigo-600 transition-colors"
							>
								{getDisplayName(type)}
							</Link>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
				...
				<AktivitasLog ref={aktivitasLogRef} lembagaType={type} lembagaId={id} />
			</div>
		</div>			<EditModal
